...
public class Homework1Grader {
	...
	private enum Category {
		...
		Planet(40), Averager(40), Style(10), Comments(10);
	...
	private void grade() {
		...
		for (Category cat : catToDeductions.keySet()) {
			...
			totalDeductionsThisCategory = Math.min(totalDeductionsThisCategory,
					cat.maxDeductions);
			sop("TOTAL DEDUCTIONS THIS CATEGORY (max=-" + cat.maxDeductions
					+ "): -" + totalDeductionsThisCategory);
			score -= totalDeductionsThisCategory;
		}
		if (score < 100)
			sop("---------------------------\n");
		if (styleDeduction > 0)
			sop("Style: -" + styleDeduction + " of "
					+ Category.Style.getMaxDeductions());
		score -= styleDeduction;
		if (commentDeduction > 0)
			sop("Comments: -" + commentDeduction + " of "
					+ Category.Comments.getMaxDeductions());
	...
	private void gradePlanet() {
		...
		try {
			Method getAllMeth = clazz.getDeclaredMethod("getAll",
					EMPTY_ARGSLIST);
	...
	private void gradeAverager() {
		...
		try {
			getAllMeth = clazz.getDeclaredMethod("getMeanPlanetaryMass",
					EMPTY_ARGSLIST);
			int methMods = getAllMeth.getModifiers();
			if (!Modifier.isPublic(methMods)) {
				deduct(Category.Averager,
						"getMeanPlanetaryMass() is not public", 15);
				return;
			}
			if (getAllMeth.getReturnType() != float.class) {
				deduct(Category.Averager,
						"getMeanPlanetaryMass() return type is not float", 25);
				return;
			}
		} catch (NoSuchMethodException x) {
		...
		try {
			...
			if (Math.abs(avg - EXPECTED_MEAN) > tolerance) {
				deduct(Category.Averager,
						"Wrong value from getMeanPlanetaryMass(): "
								+ EXPECTED_MEAN, 18);
				return;
			}
		} catch (NoSuchMethodException x) {
	...
	private class SubjectivePanel extends JPanel {
		...
		SubjectivePanel() {
			...
			for (Category cat : cats) {
				...
				JSlider slider = new JSlider(0, cat.getMaxDeductions(),
						cat.getMaxDeductions());
